...
redncost_giv_mp <- function(df, mp) {
  ...
  dt <- dt %>%
    ...
    mutate(pctred_b = wgt * 100 * (1 - ((f_mp/curr_f)^sens_exp))) %>%
...
redncost_giv_my <- function(df, my) { # function is analogous to 'redncost_giv_mp' above
  ...
  dt <- dt %>%
    mutate(pctred_b = wgt * 100 * (1 - ((f_my/curr_f)^sens_exp))) %>%
...
single_worldstate_outputs <- 
  function(dt2, n2, pctredb, pctredbl, pctredbu, reltdf) {
    samp <- lapply(dt2,
      ...
      bind_rows() %>%
      mutate(wgt = wt * (1/n2),
             pctrmsywt = wgt * (1 - ((1 - pctredfmsy)^sens_exp)),
             pctrmeywt = wgt * (1 - ((1 - pctredfmey)^sens_exp)))
